...
public class RNAdmobNativeViewManager extends ViewGroupManager<RNAdmobNativeView> {
    ...
    public void receiveCommand(RNAdmobNativeView nativeAdWrapper, int commandId, @Nullable ReadableArray args) {
        Log.d("RNAdmobNative", "==== receiveCommand START ====");
        Log.d("RNAdmobNative", "Command ID received: " + commandId);
        Log.d("RNAdmobNative", "COMMAND_LOAD_AD constant: " + COMMAND_LOAD_AD);
        Log.d("RNAdmobNative", "nativeAdWrapper is null: " + (nativeAdWrapper == null));
        Log.d("RNAdmobNative", "args is null: " + (args == null));
        
        if (args != null) {
            Log.d("RNAdmobNative", "args size: " + args.size());
            Log.d("RNAdmobNative", "args content: " + args.toString());
        }
        
        // Kiểm tra nativeAdWrapper trước
        if (nativeAdWrapper == null) {
            Log.e("RNAdmobNative", "ERROR: nativeAdWrapper is null! Cannot execute command.");
            return;
        }
        
        // Kiểm tra command ID
        if (commandId == COMMAND_LOAD_AD) {
            Log.d("RNAdmobNative", "✓ Command matches COMMAND_LOAD_AD, executing loadAd()");
            
            try {
                // Kiểm tra trước khi gọi loadAd
                Log.d("RNAdmobNative", "About to call nativeAdWrapper.loadAd()");
                nativeAdWrapper.loadAd();
                Log.d("RNAdmobNative", "✓ nativeAdWrapper.loadAd() completed successfully");
                
            } catch (Exception e) {
                Log.e("RNAdmobNative", "ERROR in loadAd(): " + e.getMessage());
                Log.e("RNAdmobNative", "Exception class: " + e.getClass().getSimpleName());
                Log.e("RNAdmobNative", "Stack trace: ", e);
            }
            
        } else {
            Log.w("RNAdmobNative", "⚠ Unknown command ID: " + commandId + " (expected: " + COMMAND_LOAD_AD + ")");
            Log.w("RNAdmobNative", "Available commands: loadAd=" + COMMAND_LOAD_AD);
        }
        
        Log.d("RNAdmobNative", "==== receiveCommand END ====");
    }
